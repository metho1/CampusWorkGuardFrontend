// src/pages/Salary/Salary.tsx
import React, {useEffect, useState} from "react";
import {Button, message, Modal, Select, Tag} from "antd";
import PageHeader from "@/components/PageHeader/PageHeader";
import SectionCard from "@/components/SectionCard/SectionCard";
import {adminGetApplicationListApi, getApplicationListApi, payDepositApi} from "@/api/application";
...
const Salary: React.FC = () => {
  ...
  const fetchApplicationList = async (pageNo = 1) => {
    try {
      ...
      if (!role) return;
      const api = role === "admin" ? adminGetApplicationListApi : getApplicationListApi;
      ...
      if (res.code === 200) {
        ...
        message.error(res.message || "获取担保岗位列表失败");
      }
    } catch {
      message.error("获取担保岗位列表失败");
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      <SectionCard
        ...
        columns={[
          ...
          {
            title: "担保金额", dataIndex: "total", render: (_: any, row: any) => {
              ...
              return (
                <>
                  <div>￥{row.total}</div>
                  <div style={{color: "#888", fontSize: 12}}>
                    {row.salaryUnit === "hour" && `${row.salary} ${map[row.salaryUnit]} × 8小时 × 50%`}
                    {row.salaryUnit === "day" && `${row.salary} ${map[row.salaryUnit]} × 22天 × 50%`}
                    {row.salaryUnit === "month" && `${row.salary} ${map[row.salaryUnit]} × 1月 × 50%`}
                  </div>
                </>
              );
            },
