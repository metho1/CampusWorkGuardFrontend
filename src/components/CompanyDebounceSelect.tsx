// src/components/CompanyDebounceSelect.tsx
// 带防抖的远程搜索下拉选择框
...
import {fetchCompanyApi} from "@/api/company.ts";
...
async function fetchSchoolList(keyword: string): Promise<{ label: string; value: string | number }[]> {
  if (!keyword) return [];
  try {
    const res = await fetchCompanyApi(keyword);
    ...
    console.error("企业搜索失败:", err);
    return [];
  }
}

const CompanyDebounceSelect: React.FC = (props) => {
  return (
    <DebounceSelect
      placeholder="企业"
...
export default CompanyDebounceSelect;
